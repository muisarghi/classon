...
function index()
	...
	<h3 class='section-title text-center'>Galeri</h3>
		...
		<div class='row'>
            ...
            $galerys=mysql_query("select kategori_galeri.id as idkat, kategori_galeri.kategori as namakategori, galeri.* from kategori_galeri  left join galeri on galeri.kat=kategori_galeri.id group by kategori_galeri.id order by galeri.id desc limit 4");
	...
	";
	$galeris=mysql_query("select kategori_galeri.id as idkat, kategori_galeri.kategori as namakategori, galeri.* from galeri left join kategori_galeri on galeri.kat=kategori_galeri.id order by galeri.id desc limit 6");
	while($galeri=mysql_fetch_array($galeris))
		{
		echo"
	...
	$waka1=mysql_query("select * from jajaran where jabatan like '%waka%' order by id asc limit 1");
	while($waka=mysql_fetch_array($waka1))
		{
		$namawaka=$waka['nama'];
		$imgwaka=$waka['img'];
		}
